		...
		try {
			...
			while ((l = inputStream.readLine()) != null) {
				...
				tempTask = new Task(FAECHER.valueOf(splittedLine[0]), splittedLine[1],
						DateHelper.getInstance().dateformatParse(splittedLine[2]), STATUS.valueOf(splittedLine[3]));
		...
		try {
			...
			for (Task t : tasks) {
				outputStream.write(t.getFach().toString() + ";" + t.getAufgabe() + ";"
						+ DateHelper.getInstance().dateFormat(t.getBisDatum()) + ";" + t.getStatus().toString() + "\n");
			}
		} catch (IOException e) {
